...
export default function TemplateDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/template", { signal: controller.signal })
      .then((r) => r.json())
      .then((data: Template[]) => {
  ...
  const autoTerbilangFields = placeholders.filter((p) => {
    const base = p.replace("TERBILANG_", "");
    return p.startsWith("TERBILANG_") || AMOUNT_FIELDS.includes(p);
  });
...
<body>
  ...
  const handleAIAssist = async () => {
    if (!aiInput.trim() || aiLoading || !template) return;
    setAiLoading(true);
    setAiMessage("");
    ...
    if (templateError) {
      return (
        <div className="max-w-5xl mx-auto px-4 py-12 text-center">
          ...
          <h1 className="text-2xl font-bold mb-3">Template Tidak Ditemukan</h1>
          <p className="text-gray-500 mb-6">
            Template surat dengan alamat tersebut tidak tersedia. Silakan pilih
            template lain dari daftar.
          </p>
          <a
            href="/template-surat"
